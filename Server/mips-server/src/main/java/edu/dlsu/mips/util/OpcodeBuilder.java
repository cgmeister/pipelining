...
public class OpcodeBuilder {
	...
	public static Opcode buildOpcode(String instruction) throws JumpAddressException, OperandException{
	...
	private static void decodeJType(Opcode opcode)throws JumpAddressException, OperandException{
		...
		if(nameIntValue%4!=0){
			...
			String opcodeBitString = BitStringUtils.doUnsignedBinarySignExtention(opcodeValue.get(tokens[0]), 6);
			opcode.setBinaryOpcode(opcodeBitString);
			String nameBitString = BitStringUtils.doUnsignedBinarySignExtention(nameIntValue, 26);
			...
			try {
				hexOpcode = BitStringUtils.doUnsignedHexSignExtension(hexOpcode, 32);
	...
	private static void decodeRType(Opcode opcode) throws OperandException{
		...
		for(int i=1; i<tokens.length; i++){
			...
			if(tokens[i]==null || tokens[i]==""){
				break;
			}else if(!Character.toString(tokens[i].charAt(0)).equalsIgnoreCase("R")){
				throw new OperandException("Register Operands must begin with R");
			}
			operands[i-1] = tokens[i];
		}
		if(operandCount!=3){
			throw new OperandException("R Types must have 3 arguments <R args0, args1, args2>");
		}
		int opcodeIntValue = opcodeValue.get(tokens[0]);
		int rdAddressIntVal = Integer.parseInt(operands[0].substring(1, operands[0].length()));
		int rsAddressIntVal = Integer.parseInt(operands[1].substring(1, operands[1].length()));
		int rtAddressIntVal = Integer.parseInt(operands[2].substring(1, operands[2].length()));
		int funcIntVal = opcodeSubValue.get(tokens[0]);
		String opcodeBitString = BitStringUtils.doUnsignedBinarySignExtention(opcodeIntValue, 6);
		opcode.setBinaryOpcode(opcodeBitString);
		String rsBitString = BitStringUtils.doUnsignedBinarySignExtention(rsAddressIntVal, 5);
		opcode.setRs(rsBitString);
		String rtBitString = BitStringUtils.doUnsignedBinarySignExtention(rtAddressIntVal, 5);
		opcode.setRt(rtBitString);
		String rdBitString = BitStringUtils.doUnsignedBinarySignExtention(rdAddressIntVal, 5);
		opcode.setRd(rdBitString);
		String zero = opcode.getZero();
		String funcBitString = BitStringUtils.doUnsignedBinarySignExtention(funcIntVal, 6);
		opcode.setFunc(funcBitString);
		String binaryOpcode = opcodeBitString+rsBitString+rtBitString+rdBitString+zero+funcBitString;
		opcode.setBinaryInstruction(binaryOpcode);
		BigInteger binaryIntVal = new BigInteger(binaryOpcode, 2);
		String hexOpcode = Long.toHexString(binaryIntVal.longValue());
		try {
	...
	private static void decodeIType(Opcode opcode) throws OperandException{
		...
		if(keyword.equals("BNEZ")){
			...
			for(int i=1; i<tokens.length; i++){
				...
				if(tokens[i]==null || tokens[i]==""){
					break;
				}else if(i==1 &&!Character.toString(tokens[i].charAt(0)).equalsIgnoreCase("R")){
					throw new OperandException("Register Operands must begin with R");
				}
				operands[i-1] = tokens[i];
			}
			if(operandCount!=3){
				throw new OperandException("BNEZ must have 3 arguments <BNEZ rs, currentPc, targetPc>");
			}
			int rsAddressIntVal = Integer.parseInt(operands[0].substring(1, operands[0].length()));
			...
			opcode.setRd(rdBitString);
			String immBitString = BitStringUtils.doUnsignedBinarySignExtention(immIntVal, 16);
			opcode.setImm(immBitString);
			String binaryOpcode = opcodeBitString+rsBitString+rdBitString+immBitString;
			opcode.setBinaryInstruction(binaryOpcode);
			BigInteger binaryIntVal = new BigInteger(binaryOpcode, 2);
			String hexOpcode = Long.toHexString(binaryIntVal.longValue());
			try {
			...
			if(operandCount!=2){
				throw new OperandException("LD/SD must have 2 arguments <LD/SD rd, offset(rs)>");
			}
		...
		//System.out.println(opcode.getHexOpCode());
			...
			for(int i=1; i<tokens.length; i++){
				...
				if(tokens[i]==null || tokens[i]==""){
					break;
				}else if(i<2 &&!Character.toString(tokens[i].charAt(0)).equalsIgnoreCase("R")){
					throw new OperandException("Register Operands must begin with R");
				}else if(i==3 &&!Character.toString(tokens[i].charAt(0)).equalsIgnoreCase("#")){
					throw new OperandException("Immediate Operands should begin with a #");
				}
				operands[i-1] = tokens[i];
				
			}
			if(operandCount!=3){
				throw new OperandException("DADDI must have 3 arguments <DADDI rd, rs, imm>");
			}
			int rdAddressIntVal = Integer.parseInt(operands[0].substring(1, operands[0].length()));
			int rsAddressIntVal = Integer.parseInt(operands[1].substring(1, operands[1].length()));
			int immIntVal = Integer.parseInt(operands[2].substring(1, operands[2].length()), 16);
			String opcodeBitString = BitStringUtils.doUnsignedBinarySignExtention(opcodeIntValue, 6);
			opcode.setBinaryOpcode(opcodeBitString);
			String rsBitString = BitStringUtils.doUnsignedBinarySignExtention(rsAddressIntVal, 5);
			opcode.setRs(rsBitString);
